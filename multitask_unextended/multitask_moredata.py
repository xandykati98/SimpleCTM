...
class CombinedMultitaskDataset(Dataset):
    """
    Combined dataset that mixes multiple image datasets for simultaneous training.
    
    Tasks:
    - mnist_digit: MNIST digit classification (10 classes)
    - mnist_even_odd: MNIST even/odd (2 classes)
    - cifar_fine: CIFAR-10 fine classification (10 classes)
    - cifar_coarse: CIFAR-10 coarse classification - Animal vs Vehicle (2 classes)
    ...
    Supports None for any dataset to train on subset of datasets.
    """
    def __init__(
        self, 
        ...
        # Calculate cumulative indices for dataset boundaries
    ...
    def _empty_labels(self) -> dict[str, int]:
        """Return labels dict with all tasks set to -1 (not applicable)."""
        return {
            'mnist_digit': -1,
            'mnist_even_odd': -1,
            'cifar_fine': -1,
...
def train_combined_multitask_model(
    ...
    """
    Training loop for MultitaskCTM on combined multi-dataset.
    Each sample is from one dataset, with masked losses for non-applicable tasks.
    
    Args:
        task_index_mapping: Maps original task index (0-5) to new model task index
    ...
    # Map dataset source to original task indices and names
    # Order: mnist_digit(0), mnist_even_odd(1), cifar_fine(2), cifar_coarse(3), fashion_fine(4), emnist_fine(5)
    original_task_names = ['mnist_digit', 'mnist_even_odd', 'cifar_fine', 'cifar_coarse', 'fashion_fine', 'emnist_fine']
    
    # Dataset to task indices mapping
    dataset_task_indices = {
        'mnist': [0, 1],    # mnist_digit, mnist_even_odd
        'cifar': [2, 3],    # cifar_fine, cifar_coarse
        'fashion': [4],     # fashion_fine
        'emnist': [5],      # emnist_fine
    }
                ...
                if stacked_images[dataset_name] is not None:
                    ...
                    for original_idx in task_indices:
                        if original_idx in task_index_mapping:
                            ...
                            if (target >= 0).any():
                                output = model(data, task_idx=model_task_idx)
                                loss = criterion(output, target)
...
def custom_collate_fn(batch: list) -> tuple:
    ...
    # Separate images by dataset type since they have different shapes
...
def train_combined(device: torch.device, selected_tasks: list[str], epochs: int, n_neurons: int):
    ...
    # MNIST transforms (also used for Fashion MNIST and EMNIST)
    ...
    train_dataset = CombinedMultitaskDataset(
        ...
        fashion_dataset=fashion_train,
        emnist_dataset=emnist_train
    )
    test_dataset = CombinedMultitaskDataset(
        mnist_dataset=mnist_test, 
        cifar_dataset=cifar_test,
        fashion_dataset=fashion_test,
        emnist_dataset=emnist_test
    )
    ...
    # Get sample image for visualization (get first available)
...
Examples:
  ...
  python multitask_extended.py --tasks cifar_fine
  
  # Train on MNIST digit and CIFAR fine
  python multitask_extended.py --tasks mnist_digit cifar_fine
  
  # Train on Fashion MNIST and EMNIST
  python multitask_extended.py --tasks fashion_fine emnist_fine
  
  # Train on all tasks
  python multitask_extended.py --tasks mnist_digit mnist_even_odd cifar_fine cifar_coarse fashion_fine emnist_fine
        """
    )
